...
(function() {
	...
	*/
	Client.prototype.aggregators = function( clbk ) {
		...
		fcn = getData( url, function onResponse( error, aggregators ) {
			if ( error ) {
				clbk( error );
				return;
			}
	...
	*/
	Client.prototype.metrics = function( clbk ) {
		...
		url = 'http://'+this._host+':'+this._port+'/api/suggest?type=metrics&max=1000000000000&q=';

		fcn = getData( url, function onResponse( error, metrics ) {
			if ( error ) {
				clbk( error );
				return;
			}
