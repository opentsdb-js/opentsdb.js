...
(function() {
	...
	* @returns {Client|Boolean} Client instance or rate flag
	*/
	...
	*
	* @param {Object} [options] - rate options object; limited to the following properties: counter, counterMax, resetValue.
	* @returns {Client|Object} Client instance or rate options
	*/
	Query.prototype.rateOptions = function( options ) {
		...
		if ( counter && typeof counter !== 'boolean' ) {
		...
		if ( counterMax && ( typeof counterMax !== 'number' || counterMax !== counterMax ) ) {
		...
		if ( resetValue && ( typeof resetValue !== 'number' || resetValue !== resetValue ) ) {
	...
	*	Tag value setter and getter. If a value is supplied, sets the tag value. If not value is supplied, gets the tag value. If the tag name has not be set, getting the tag value returns undefined.
	*
	* @param {String} tag - tag name
	* @param {String} [value] - tag value; e.g., 'foo', 'foo|bar', or '*', where '*' means to request data for all tag values for a given tag name
	* @returns {Client|String|undefined} Client instance or tag value
	*/
	Query.prototype.tag = function( tag, value ) {
		var numArgs = arguments.length;
		if ( !numArgs ) {
			throw new Error( 'tag()::insufficient input arguments. Must provide a tag name and/or value.' );
		}
	...
	/**
	* METHOD: tags()
	*	Returns all the current tags. NOTE: the returned object does not correspond to the internal object. Hence, the internal object should be treated as not mutable.
	*
	* @returns {Object} tags
	*/
